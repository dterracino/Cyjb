...
namespace Cyjb.Configurations
{
		...
		where TElement : ConfigurationElement
	{
		...
		/// <summary>
		/// 从特定的 <see cref="Array"/> 索引处开始，将 <see cref="ConfigurationElementCollection{TElement}"/> 
		...
		/// <exception cref="ArgumentException"><see cref="ConfigurationElementCollection{TElement}"/>
		/// 中的元素数目大于从 <paramref name="arrayIndex"/> 到目标 <paramref name="array"/> 
		/// 末尾之间的可用空间。</exception>
		/// <overloads>
		/// <summary>
		/// 将 <see cref="ConfigurationElementCollection{TElement}"/> 的元素复制到一个 <see cref="Array"/> 中。
		/// </summary>
		/// </overloads>
